...
class Background(Scenario):
    ...
    def create_instance(self):
        ...
        """
        background = Background(self.keyword, self.sentence, self.path, self.line, self.parent)

        for step in self.all_steps:
            # FIXME(TF): move to Step.copy
            step_copy = Step(step.id, step.sentence, step.path, step.line, step.parent, step.runable)
