...
def console_writer_before_each_step(step):
    ...
    output += u"\r        {0}{1}".format(get_id_sentence_prefix(step, colorful.bold_brown), colorful.bold_brown(step.sentence))
...
def console_writer_after_each_step(step):
    ...
    output = u"{0}        {1}{2}".format(line_jump_seq, get_id_sentence_prefix(step, colorful.bold_cyan), color_func(step.sentence))
    ...
    if step.state == step.State.FAILED:
        if world.config.with_traceback:
            output += u"\n          {0}{1}".format(get_id_padding(len(step.parent.steps) - 2), "\n          ".join([colorful.red(l) for l in step.failure.traceback.split("\n")[:-2]]))
        output += u"\n          {0}{1}: {2}".format(get_id_padding(len(step.parent.steps) - 2), colorful.bold_red(step.failure.name), colorful.red(step.failure.reason))
...
def console_writer_after_each_scenario(scenario):
    ...
    if isinstance(scenario, ScenarioOutline):
        ...
        if scenario.state == Step.State.FAILED:
            ...
            if world.config.with_traceback:
                output += u"\n          {0}{1}".format(get_id_padding(len(scenario.parent.scenarios)), "\n          ".join([colorful.red(l) for l in failed_step.failure.traceback.split("\n")[:-2]]))
            output += u"\n          {0}{1}: {2}".format(get_id_padding(len(scenario.parent.scenarios)), colorful.bold_red(failed_step.failure.name), colorful.red(failed_step.failure.reason))
    elif isinstance(scenario.parent, ScenarioLoop):
        colored_pipe = colorful.bold_white("|")
        color_func = get_color_func(scenario.state)
