...
def syslog_writer_before_each_feature(feature):
    ...
    """
    log("begin feature {}:{} {}".format(world.config.marker, feature.id, feature.sentence))
...
def syslog_writer_after_each_feature(feature):
    """
        Writes the feature to the syslog
    """
    log("end feature {}:{} {}".format(world.config.marker, feature.id, feature.sentence))
...
def syslog_writer_before_each_scenario(scenario):
    ...
    """
    log("begin scenario {}:{}.{} {}".format(world.config.marker, get_scenario_feature(scenario).id, scenario.id, scenario.sentence))
...
def syslog_writer_after_each_scenario(scenario):
    """
        Writes the scenario to the syslog
    """
    log("end scenario {}:{}.{} {}".format(world.config.marker, get_scenario_feature(scenario).id, scenario.id, scenario.sentence))
...
def syslog_writer_before_each_step(step):
    ...
    """
    log("begin step {}:{}.{}.{} {}".format(world.config.marker, get_scenario_feature(step.parent).id, step.parent.id, step.id, step.sentence))
...
def syslog_writer_after_each_step(step):
    """
        Writes the step to the syslog
    """
    log("{} step {}:{}.{}.{} {}".format(step.state, world.config.marker, get_scenario_feature(step.parent).id, step.parent.id, step.id, step.sentence))
