...
class FeatureFileParser:
    def __init__(self, grammerfile: Path = None) -> None:
        ...
        self._parser = Lark.open(str(grammerfile), parser="lalr", transformer=self._transformer)
    ...
    def parse_contents(self, featurefile_path: Path, featurefile_contents: str):
        ...
        self._transformer.prepare(featurefile_path, featurefile_contents, self._current_feature_id)

        try:
            ast = self._parser.parse(featurefile_contents)
        except UnexpectedInput as exc:
            exc_class = exc.match_examples(self._parser.parse, {
                RadishMissingFeatureShortDescription: RadishMissingFeatureShortDescription.examples,
                RadishMissingRuleShortDescription: RadishMissingRuleShortDescription.examples,
                RadishMissingScenarioShortDescription: RadishMissingScenarioShortDescription.examples,
                ...
                RadishScenarioOutlineWithoutExamples: RadishScenarioOutlineWithoutExamples.examples,
                RadishScenarioOutlineExamplesMissingOpeningVBar: RadishScenarioOutlineExamplesMissingOpeningVBar.examples,
                RadishMultipleBackgrounds: RadishMultipleBackgrounds.examples,
                RadishStepDataTableMissingClosingVBar: RadishStepDataTableMissingClosingVBar.examples,
                RadishScenarioLoopMissingIterations: RadishScenarioLoopMissingIterations.examples,
                RadishScenarioLoopInvalidIterationsValue: RadishScenarioLoopInvalidIterationsValue.examples,
                RadishScenarioOutlineExamplesMissingClosingVBar: RadishScenarioOutlineExamplesMissingClosingVBar.examples,
                RadishScenarioOutlineExamplesInconsistentCellCount: RadishScenarioOutlineExamplesInconsistentCellCount.examples
            })
            if not exc_class:
                raise
            raise exc_class(exc.get_context(featurefile_contents), exc.line, exc.column)
        except RadishSyntaxError as exc:
            raise
        return ast
