...
@pytest.mark.parametrize('language, feature_keyword, scenario_keyword', [
    ('en', 'Feature', 'Scenario'),
    ('de', 'Funktionalität', 'Szenario')
])
...
@pytest.mark.parametrize('parser, expected_feature_tags, expected_scenarios_tags', [
    (['tags-feature'], [Tag('foo'), Tag('bar')], [[]]),
    (['tags-scenario'], [], [[Tag('foo'), Tag('bar')]]),
    (['tags-everywhere'], [Tag('foo'), Tag('bar')], [
        [Tag('regular_scenario')], [Tag('scenario_outline')], [Tag('scenario_loop')]
    ]),
...
@pytest.mark.parametrize('parser, expected_scenarios', [
    (
        ('tag-inheritance', [], {'tag_expr': tagexpressions.parse('some_feature')}), 3
    ),
    (
        ('tag-inheritance', [], {'tag_expr': tagexpressions.parse('some_feature and good_case')}), 2
    ),
    (
        ('tag-inheritance', [], {'tag_expr': tagexpressions.parse('some_feature and bad_case')}), 1
    ),
    (
        ('tag-inheritance', [], {'tag_expr': tagexpressions.parse('some_feature and not bad_case')}), 2
    ),
    (
        ('tag-inheritance', [], {'tag_expr': tagexpressions.parse('good_case or bad_case')}), 3
    ),
], indirect=['parser'])
...
def test_parse_scenario_sentence_duplicate(parser):
    ...
    assert str(exc.value).startswith(
        "Scenario with name 'A great name for a Scenario' "
        "defined twice in feature")
...
@pytest.mark.parametrize('parser, expected_error_msg', [
    (
        ('precondition-unknown-scenario',), "Cannot import precondition scenario 'Unknown Scenario' from feature"
    ),
    (
        ('precondition-unknown-scenario-same-feature',), "Cannot import precondition scenario 'Unknown Scenario' from feature"
    ),
    (
        ('precondition-recursion',), 'Your feature'
    ),
    (
        ('precondition-malformed',), "Scenario @precondition tag must have argument in format: 'test.feature: Some scenario'"
    )
], ids=[
    'Unknown Scenario',
