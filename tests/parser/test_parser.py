...
@pytest.mark.xfail(reason="Not implemented yet. Currently the second Feature is parsed as description")
...
def test_parse_multiline_description_with_blanklines_from_feature(parser):
    ...
    assert ast.description == [
        "This is the description", "on multiple lines."
        "",
        "It even contains blank lines",
        "which is great for readability."
    ]
...
@pytest.mark.parametrize("feature_file", [
    """
        Feature: My Feature
    ...
    pytest.param("""
        Feature: My Feature
        Rule: My Rule
    """, marks=pytest.mark.xfail(reason="A Feature without description must have a blank line before the first block"))
], ids=[
...
@pytest.mark.xfail(reason="RadishMultipleBackgrounds is not matched. Probably a bug in lark?")
...
@pytest.mark.xfail(reason="RadishMisplacedBackground is not matched. Probably a bug in lark?")
...
def test_parse_fail_for_inconsistent_cell_count_in_step_data_table(parser, feature_file):
...
def test_parser_built_examples_for_scenario_loop_have_copied_steps(parser):
    ...
    assert len(ast.rules[0].scenarios[0].steps) == 1
    assert ast.rules[0].scenarios[0].steps[0].text == "there is a Step"
    assert ast.rules[0].scenarios[0].examples[0].steps[0] is not ast.rules[0].scenarios[0].steps[0]
    assert ast.rules[0].scenarios[0].examples[1].steps[0] is not ast.rules[0].scenarios[0].steps[0]
...
def test_parse_fail_for_missing_closing_vbar_in_scenario_outline_examples(parser, feature_file):
...
def test_parse_fail_for_missing_opening_vbar_in_scenario_outline_examples(parser, feature_file):
...
def test_parse_fail_for_inconsistent_cell_count_in_scenario_outline_examples(parser, feature_file):
    """
...
def test_parser_replace_examples_parameter_in_scenario_outline_examples(parser):
    ...
    assert ast.rules[0].scenarios[0].examples[0].steps[2].text == "there is a foo-bar Step"
    ...
    assert ast.rules[0].scenarios[0].examples[1].steps[2].text == "there is a meh-bla Step"
