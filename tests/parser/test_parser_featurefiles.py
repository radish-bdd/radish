...
@pytest.mark.parametrize("feature_file_name", [
    "feature-only.feature",
    "feature-only-no-trailing-blankline.feature"
], ids=[
...
def test_parse_feature_with_multiple_scenarios_with_steps(parser):
    ...
    assert first_scenario.short_description == "A simple Scenario containing three Steps"
    ...
    assert second_scenario.short_description == "Another simple Scenario containing three Steps"
...
def test_parse_feature_without_description_with_scenario_with_steps(parser):
    """The parser should be able to parse a Feature without a Description but with a single Scenario with Steps"""
    # given
    feature_file_path = FEATURE_FILES_DIR / "feature-without-description-scenario-steps.feature"
...
def test_parse_feature_with_scenario_outline(parser):
    ...
    assert scenario_outline.short_description == "A simple Scenario containing three Steps"
...
def test_parse_feature_with_scenario_outline_escaped_vbars(parser):
    ...
    # given
    feature_file_path = FEATURE_FILES_DIR / "feature-scenario-outline-escaped-vbars.feature"
...
def test_parse_feature_with_background_and_scenario_outline(parser):
    ...
    # given
    feature_file_path = FEATURE_FILES_DIR / "feature-background-and-scenario-outline.feature"
...
def test_parse_feature_with_background_and_scenario_loop(parser):
    ...
    # given
    feature_file_path = FEATURE_FILES_DIR / "feature-background-and-scenario-loop.feature"
...
def test_parse_feature_with_scenario_with_steps_doc_string(parser):
    ...
    assert scenario.steps[0].text == "the webservice is started"
    assert scenario.steps[1].keyword == "When"
    assert scenario.steps[1].text == "the /foo/bar route is queried with the following body"
...
def test_parse_feature_with_scenario_with_steps_data_table(parser):
    ...
    assert scenario.steps[2].data_table == [
        ["200"],
        ["404"]
    ]
...
def test_parse_feature_with_scenario_with_steps_data_table_escaped(parser):
    """The parser should be able to parse a Feature with a Scenario and Steps with data tables having escaped VBARs"""
    # given
    feature_file_path = FEATURE_FILES_DIR / "feature-scenario-steps-data-tables-escaped-vbars.feature"
...
def test_parse_feature_with_background_and_rule_with_scenario(parser):
    ...
    # given
    feature_file_path = FEATURE_FILES_DIR / "feature-background-and-rule-scenario.feature"
...
def test_parse_feature_with_background_and_multiple_rules_with_scenarios(parser):
    """The parser should be able to parse a Feature with a Background and multiple Rules with a Scenario"""
    # given
    feature_file_path = FEATURE_FILES_DIR / "feature-background-and-multiple-rules-scenario.feature"
