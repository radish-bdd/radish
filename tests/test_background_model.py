...
@pytest.mark.parametrize("given_step_states, expected_state", [
    (
        [State.PASSED, State.PASSED, State.PASSED],
        State.PASSED
    ),
    (
        [State.PASSED, State.UNTESTED, State.PASSED],
        State.UNTESTED
    ),
    (
        [State.PASSED, State.SKIPPED, State.UNTESTED],
        State.SKIPPED
    ),
    (
        [State.PASSED, State.PENDING, State.SKIPPED],
        State.PENDING
    ),
    (
        [State.PASSED, State.FAILED, State.PENDING],
        State.FAILED
    ),
    (
        [State.PASSED, State.RUNNING, State.FAILED],
        State.RUNNING
    ),
], ids=[
...
def test_background_should_correct_state(given_step_states, expected_state, mocker):
    ...
    # given
    background = Background("Background", None, None, [mocker.MagicMock(state=s) for s in given_step_states])
